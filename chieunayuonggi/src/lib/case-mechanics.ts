...
// Typical Vietnamese afternoon-drink price sits around 25–30k VND per cup.
// This is a rough anchor for the spin distribution, not a survey figure.
export const TARGET_DRINK_PRICE = 30;
// Log-price normalization anchor, scaled to the drink price range (vs. 50 for meals).
const NORMALIZE_PRICE = 25;
// Product choice: spread in log-price space, not measured buyer behavior.
export const LOG_PRICE_SPREAD = .4;
...
type PricedDrink={price:number;rarity:number};
export function createFoodSelector<T extends PricedDrink>(population:T[],target=TARGET_DRINK_PRICE){
 if(!population.length)throw new Error('No drinks in population');
 if(!Number.isFinite(target)||target<=0)throw new Error('Invalid target');
 if(population.some(f=>!Number.isFinite(f.price)||f.price<=0))throw new Error('Invalid drink price');
 const min=Math.min(...population.map(f=>f.price)),max=Math.max(...population.map(f=>f.price));
 if(target<min||target>max)throw new Error('Target mean is outside feasible drink prices');
 // Equal total prior weight per distinct price, split among drinks at that price.
 ...
 const logs=population.map(f=>Math.log(f.price/NORMALIZE_PRICE));
 ...
 function weighted(items:T[]){
  if(!items.length)throw new Error('No eligible drinks');
  const w=items.map(f=>{const p=probabilities.get(f);if(p===undefined)throw new Error('Unknown drink');return p});
  const sum=w.reduce((s,p)=>s+p,0);if(sum<=0)throw new Error('Eligible drinks have no probability');
  return {w,sum};
 }
...
export function priceRarity(priceInThousands:number){return priceInThousands<=15?0:priceInThousands<=25?1:priceInThousands<=35?2:priceInThousands<=45?3:4}
